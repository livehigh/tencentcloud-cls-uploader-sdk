...
|  [maxRetainDuration?](./tencentcloud-cls-uploader-sdk.iclientconfig.maxretainduration.md) |  | number | _(Optional)_ 未上传的缓存日志超过该时间则上传（单位: s，默认 30s） |
|  [maxRetainSize?](./tencentcloud-cls-uploader-sdk.iclientconfig.maxretainsize.md) |  | number | _(Optional)_ 未上传的缓存日志数量超过该数值则上传（默认 20 条） |
